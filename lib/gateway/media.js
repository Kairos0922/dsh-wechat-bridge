...
import { assertCdnUrl, MEDIA_DOWNLOAD_MAX_BYTES, WEIXIN_CDN_BASE_URL } from "./types.js";
 ...
 */
async function fetchCdnWithRedirects(startUrl, extraTrustedHosts, fetchFn) {
    let url = startUrl;
    for (let hop = 0;; hop++) {
        ...
        if (res.status >= 300 && res.status < 400) {
            ...
            if (!location)
                throw new Error(`CDN redirect ${res.status} without Location header`);
...
export async function downloadMediaObject(params) {
    ...
    if (!keyInput)
        throw new Error('media item has no aes key');
    const key = parseAesKey(keyInput);

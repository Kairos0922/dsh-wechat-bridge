 ...
 */
function maskEncryptedQueryParam(s) {
    if (s.length <= 20)
        return `<eqp${s.length}:redacted>`;
    return `<eqp${s.length}:${s.slice(0, 12)}…${s.slice(-6)}>`;
}
const EQP_KEY_RE = /"((?:encrypt_query_param|thumb_encrypt_query_param))":"([^"]{4,})"/g;
function redactEncryptedQueryParams(text) {
    return text.replace(EQP_KEY_RE, (_m, key, val) => `"${key}":"${maskEncryptedQueryParam(val)}"`);
}
...
export function debugLogMediaCapture(event) {
    try {
        ...
        fs.mkdirSync(path.dirname(file), { recursive: true });
        const line = redactEncryptedQueryParams(JSON.stringify({ ts: new Date().toISOString(), msgId: event.msgId ?? null, item: event.item })) + '\n';
        fs.appendFileSync(file, line);

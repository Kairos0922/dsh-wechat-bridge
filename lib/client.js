...
function WechatBridgePanel(props) {
    ...
    const svgDataUrl = qr
        ...
        : null;
    return h('section', { style: css.section }, h('div', { style: css.row }, h('h3', { style: css.title }, t('title')), status?.paired
        ? h('span', { style: css.pill }, t('paired'))
        : h('span', { style: css.pillError }, t('unpaired'))), h('div', { style: css.card }, h('div', null, h('span', { style: css.label }, `${t('gatewayStatus')} · ${status?.status ?? '…'}`), status?.pairingMessage ? h('p', { style: css.muted }, status.pairingMessage) : null), h('div', null, h('span', { style: css.label }, t('accountId')), h('p', { style: css.value }, status?.accountId ?? '—')), h('div', null, h('span', { style: css.label }, t('allowFrom')), h('div', null, (status?.allowFrom ?? []).length > 0
        ? status.allowFrom.map((id) => h('span', { key: id, style: css.chip }, id))
        : h('p', { style: css.muted }, t('emptyAllowlist')))), h('div', null, h('span', { style: css.label }, `${t('modes')}${status?.defaultMode ? ` · ${t('defaultMode')}: ${status.defaultMode}` : ''}`), h('div', null, (status?.modes ?? []).length > 0
        ? status.modes.map((id) => h('span', { key: id, style: css.chip }, id))
        : h('p', { style: css.muted }, '—')))), h('div', { style: css.card }, h('div', { style: css.row }, h('h4', { style: css.title }, t('pair')), h('button', { style: css.button, onClick: () => void pair(), disabled: pairing }, pairing ? t('pairing') : t('pair'))), svgDataUrl ? h('img', { src: svgDataUrl, style: css.qr, alt: 'WeChat QR' }) : null, h('p', { style: css.muted }, t('pairHint')), error ? h('p', { style: css.error }, `${t('requestFailed')}: ${error}`) : null), h('div', { style: css.card }, h('h4', { style: css.title }, t('helpTitle')), h('pre', { style: css.pre }, t('help'))));
}
// ---------------------------------------------------------------- plugin

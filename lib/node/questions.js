 ...
 */
export function parseQuestionReply(raw, question) {
    const text = raw.trim();
    if (text.length === 0)
        return { kind: 'none' };
    if (text.startsWith('/'))
...
export function buildQuestionPrompt(pending, timeoutSec) {
    ...
    if (question.detail) {
        ...
        lines.push(detail.length > DETAIL_MAX_CHARS
 ...
 */
export function attachUserQuestionBridge(node) {
    const listener = async (request, next) => {
        ...
        return await new Promise((resolve, reject) => {
            ...
            const pending = {
                ...
                submit(text) {
                    ...
                    const item = { id: question.id, selected: reply.selected };
                    if (reply.custom !== undefined)
                        item.custom = reply.custom;
                    answers.push(item);
                    pending.index += 1;

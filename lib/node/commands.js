...
import { lastSessionEvent, reversedSessionEvents, sessionEvents } from "../session-events.js";
import { listModes } from "./presets.js";
import { latestContextInput, sendTextToPeer, textOfAssistantMessage } from "./outbound.js";
import fs from 'node:fs';
import path from 'node:path';
import { buildTranscript, exportsDir, writeExportFile } from "./exports.js";
import { renderCardToPng } from "./card.js";
 ...
 * Exported for tests.
 */
export function validateVideoPath(node, resolvedPath) {
    ...
    if (stat.size > VIDEO_MAX_BYTES)
        return `视频超过 10MB 限制（${Math.round(stat.size / 1024 / 1024)}MB）`;
    const roots = (node.resolved.videoRoots ?? [node.resolved.cwd ?? process.cwd(), node.resolved.mediaDir ?? defaultMediaDir()])
        .map((root) => {
        try {
    ...
    if (!inside)
        return '只允许发送工作区/媒体目录内的视频（可用 videoRoots 配置）';
    // Magic check: ISO BMFF files start with a box whose bytes 4..8 are "ftyp".
    if (stat.size >= 12) {
        ...
        try {
            ...
            if (header.toString('ascii', 4, 8) !== 'ftyp')
                return '文件内容不是有效的 MP4（缺少 ftyp 头）';
        }
        finally {
            fs.closeSync(fd);
        }
...
function timeAgo(epochMs) {
    ...
    if (delta < 60_000)
        return '刚刚';
    if (delta < 3_600_000)
        return `${Math.round(delta / 60_000)}m 前`;
    if (delta < 86_400_000)
        return `${Math.round(delta / 3_600_000)}h 前`;
    return `${Math.round(delta / 86_400_000)}d 前`;
}
...
export function listSessions(node) {
...
export async function parseNewArgs(node, rest) {
    const first = rest[0] ?? '';
    if (first) {
 ...
 */
export function renderModesList(modes, defaultMode, menuTimeoutSec = 60) {
...
export const COMMANDS = [
    {
        ...
        run: async (node, peerId) => {
            ...
            node.registerMenu(peerId, 'mode', modes.map((m) => ({ label: m.name ?? m.id, value: m.id })));
    ...
    },
    {
        ...
        run: async (node, peerId, args) => {
            ...
            await sendTextToPeer(node, peerId, `✅ 已切换到会话 #${index}`, { kind: 'system' });
        },
    },
    {
        id: 'stop',
    ...
    },
    {
        ...
        run: async (node, peerId) => {
            ...
            const queueLine = paused !== null && paused > Date.now()
            ...
            const pairedLine = pairedIds.length > 0
                ? `扫码配对: ${pairedIds.length} 人${pairedIds.includes(peerId) ? '（含你）' : ''}`
                : '扫码配对: 无';
            const trustLine = allowFrom.length > 0
                ? `${pairedLine} · allowFrom ${allowFrom.length} 人`
                : pairedLine;
            const pendingLine = node.pendingTrustUserId !== null ? `\n⏳ 有待确认的扫码用户（请在设置面板确认）` : '';
            await sendTextToPeer(node, peerId, `📊 状态\n会话: ${session.id}\nagent: ${agent?.status ?? 'idle'}\n事件: ${session.seq} 条 · 最近: ${reason}\n模型偏好: ${modelLine}\n工作区偏好: ${cwdLine}\n出站: ${queueLine}${usage ? `\n本轮 token: ${usage}` : ''}\n${trustLine}${pendingLine}\n📷 发图通道: 后端受限（入站图片正常）`, { kind: 'system' });
        },
    },
    {
        id: 'model',
        ...
        run: async (node, peerId, args) => {
            ...
            if (!arg) {
                ...
                node.registerMenu(peerId, 'provider', providers.map((p) => ({ label: p.id, value: p.id })));
                await sendTextToPeer(node, peerId, `🤖 选择供应商（回复编号，0 取消）：\n${providers.map((p, i) => `${i + 1}. ${p.id}`).join('\n')}\n\n${current}`, { kind: 'system' });
                return;
            }
            if (arg === 'default') {
    ...
    },
    {
        ...
        run: async (node, peerId, args) => {
            const registry = node.ctx.get('workspaceRegistry');
            const prefs = node.state.getPrefs(peerId);
            ...
            if (!arg) {
                ...
                node.registerMenu(peerId, 'workspace', workspaces.map((w) => ({ label: `${w.title}（${w.path}）`, value: w.path })));
                await sendTextToPeer(node, peerId, `📂 选择工作区（回复编号，0 取消）：\n${workspaces.map((w, i) => `${i + 1}. ${w.title}\n   ${w.path}`).join('\n')}\n\n${current}`, { kind: 'system' });
                return;
            }
            if (arg === 'default') {
    ...
    },
    {
        ...
        run: async (node, peerId) => {
            ...
            try {
                if (registry)
                    await registry.archiveSession(session.id);
                // releaseSession additionally marks the session released: it is then
                // permanently excluded from orphan adoption — a closed session never
                // silently changes hands to another peer later.
                node.releaseSession(peerId);
                await sendTextToPeer(node, peerId, `🗂 已归档会话：${sessionLabel(session)}`, { kind: 'system' });
            }
            catch (err) {
...
function tokenUsageSummary(session) {
    let last = null;
    for (const event of sessionEvents(session)) {
...
export function helpText(commandId) {
    if (commandId) {
        const spec = COMMANDS.find((c) => c.id === commandId);
        if (!spec)
            return `❓ 没有命令 /${commandId}。回复 /help 查看全部。`;
        return `🤖 /${spec.id}\n用法: ${spec.usage}\n\n${spec.detail}`;
    }
...
COMMANDS.push({
    ...
    id: 'video',
    summary: '发送本地视频文件（.mp4，限工作区/媒体目录）',
    usage: '/video <本地路径>',
    detail: '把工作区或媒体目录里的 .mp4 文件作为微信视频消息发送（≤10MB）。安全限制：只允许配置的工作区根目录' +
        '（videoRoots）内的普通文件，防止任意本机文件被读取并外发。',
    run: async (node, peerId, args) => {
        const target = args[0];
        if (!target) {
        ...
        const rejected = await validateVideoPath(node, resolved);
        if (rejected !== null) {
...
export async function routeCommand(node, peerId, text) {
    ...
    if (!spec) {
        await sendTextToPeer(node, peerId, `❓ 未知命令 /${command}\n${helpText()}\n\n提示：以 // 开头可把 / 开头的内容原样发给 agent。`, { kind: 'system' });
        return 'handled';
    }

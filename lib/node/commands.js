 ...
 */
export function parseNewArgs(node, rest) {
    const first = rest[0] ?? '';
...
function renderSessions(node) {
    const sessions = listSessions(node);
    if (sessions.length === 0)
        return '📋 没有会话。发送 /new [模式] <prompt> 开始。';

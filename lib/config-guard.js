...
export function assertNoUnknownKeys(config, known, label) {
    if (!config)
        return;
    const unknown = Object.keys(config).filter((k) => !known.has(k));
    if (unknown.length === 0)
        return;
    throw new Error(`[dsh-wechat-bridge] ${label}: unknown config key(s): ${unknown.join(', ')}` +
...
export function assertCleanBaseUrl(raw, label) {
    if (!raw)
        return;
    let parsed;
    try {

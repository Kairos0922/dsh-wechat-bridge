# DSH 集成说明（可选增强：origin 徽标）

> **插件本身零宿主依赖**：默认（`originBadge: false`）不写 origin 头，任何 DSH
> 部署开箱即用，升级永不破坏。本页描述的补丁只服务于**可选项**：开启
> `originBadge: true` 后侧栏显示 🟢 来源徽标——需要宿主校验层接受
> `origin='wechat'`。开启后若宿主不支持（补丁缺失/DSH 升级），桥会自动降级为
> 普通会话（最多没有徽标，功能与持久化不受影响）。
>

...
pnpm verify                   # 期望：VERIFY_EXIT=0，64 项测试全绿
scripts/dry-run.sh --check    # 期望：状态端点健康、7 个模式、白名单正确
```
...
#   期待：status=polling、paired=true、modes 为对象数组（7 项）、outbox.pending=0
```
...
| # | 操作 | 期待 |
|---|---|---|
| 1 | `/modes` | 7 个模式，中文名+说明，每项下有一行 `/new <id>`；`life-butler（当前默认）` 带标记；底部有偏好行与 /help 提示 |
...
| 7 | `/new life-finance 你好` | 新会话带模式标签；**DSH Web 侧栏出现 `🟢 微信 ·` 标题** |
| 8 | `/help` 与 `/help model` | 12 条命令总览 / 单命令详情 |
| 9 | `/retry` | `🔁 已重新提交上次任务` |

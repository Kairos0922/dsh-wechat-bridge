...
| `/file <路径>` | 把本机文档文件作为微信附件发送（pptx/pdf/docx/xlsx/md/txt/csv/zip/图片；限 `fileRoots` 根目录，隐藏路径段拒绝，≤25MB） |
...
| `questionTimeoutSec` | 1800 | `ask_user_question` 提问等待超时（超时按「未回答」报给 agent）。提问会**阻塞整个 turn**，该值是兜底上限，绝不无限挂起 |
...
| `fileRoots` | 沿用 `videoRoots` 缺省 | `/file` 可读取的根目录白名单；隐藏路径段一律拒绝，≤25MB |
| `sessionWindowSendMax` | 10 | 服务器会话窗口出站配额上限（实测约 10 条/用户入站窗口；最终答案/审批等 must 消息豁免；0=关闭会计） |
| `inboundDebounceMs` | 2000 | 同一会话快速连续文本合并为一个 agent turn；媒体立即冲刷；0=关闭 |
| `allowGroups` | `[]` | 群聊两级白名单（腾讯暂未向机器人开放群事件，待用） |
| `botAgent` | 包名/版本 | 上报给微信服务器的 bot_agent（UA 语法自动清洗、256B 上限；仅观测用途，不影响鉴权/路由） |
...
- **限流/会话窗口配额**：微信通道无限流公开数字（`ret=-2` 的 errmsg 区分限流与窗口配额耗尽，见 `docs/protocol.md §5`）。
  - **会话窗口配额（2026-08-19 实测）**：服务器对每个用户入站窗口允许约 **10 条成功出站**，超出后一律 `prepare failed`，直到你下一条消息重置窗口。桥端对策：心跳/todo/上下文行在窗口见底时自动让位（`HEARTBEAT_QUOTA_RESERVE=3`），配额留给最终答案；**最终答案、审批提示、出错/停止通知为最高优先级（must），不受配额限制**。
  - **长任务默认静音**：中间工具叙述不再推送微信；最终答案在任务结束时一次送达；执行中仅低频「🔄 仍在处理中」心跳（120s）+「正在输入」指示。
  - **关键消息必达**：审批提示、最终答案、出错/停止通知发送失败不放弃——通道恢复（你下一条消息）时自动补发；多分块答案任一块失败会整段重推。
  - 请避免高频连续发送，可能触发服务端限流。

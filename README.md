...
- **零配置上手**：装好扫码即用；直接发消息自动创建会话（默认模式），无需手动建会话
...
- **安全边界**：白名单外的消息只记日志、绝不喂给模型（可选 `notifyRejected` 提醒）；危险操作经审批，微信里 `/yes` `/no` 或回复编号即决
...
| `/model` | 查看/切换模型（对 /new 生效；`/model default` 恢复默认） |
| `/workspace` | 查看/切换工作区（对 /new 生效；`/workspace default` 恢复默认） |

 ...
 * Exported for tests.
 */
export function validateVideoPath(node: WechatBridgeNode, resolvedPath: string): string | null {
  if (path.extname(resolvedPath).toLowerCase() !== '.mp4') return '仅支持 .mp4 视频文件'
  let stat: fs.Stats
  let real: string
  try {
  ...
  if (stat.size > VIDEO_MAX_BYTES) return `视频超过 10MB 限制（${Math.round(stat.size / 1024 / 1024)}MB）`
  ...
  if (!inside) return '只允许发送工作区/媒体目录内的视频（可用 videoRoots 配置）'
  ...
  if (stat.size >= 12) {
    ...
    try {
      ...
      if (header.toString('ascii', 4, 8) !== 'ftyp') return '文件内容不是有效的 MP4（缺少 ftyp 头）'
    } finally {
      fs.closeSync(fd)
    }
  }
...
COMMANDS.push(
  ...
  },
  {
    ...
    usage: '/video <本地路径>',
    detail:
      '把工作区或媒体目录里的 .mp4 文件作为微信视频消息发送（≤10MB）。安全限制：只允许配置的工作区根目录' +

...
export function selectExpiredFiles(
...
export function collectFiles(dir: string): Array<{ path: string; mtimeMs: number }> {
 ...
 */
export function attachMediaRetention(node: WechatBridgeNode): () => void {
  const mediaDir = node.resolved.mediaDir ?? defaultMediaDir()
  if (node.resolved.mediaRetentionDays <= 0) return () => {}

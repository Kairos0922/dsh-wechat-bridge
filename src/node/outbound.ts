...
export function isProgressTool(node: WechatBridgeNode, name: string): boolean {
...
interface DigestState {
  ...
  todoHash: string
  lastTickKey: string
  /** callId → tool name, so interleaved cards keep their own label. */
...
export function attachSessionOutbound(node: WechatBridgeNode): () => void {
  ...
  const tickKey = (state: DigestState) => `${state.reasoningChars}|${state.toolCount}|${state.lastTool ?? ''}`

 ...
 */
export function sessionIdCreatedAt(id: string): number {
...
export class WechatBridgeNode {
    ...
    if (!agent) {
      ...
      if (!agent) {
        ...
        if (restoreAttempted) {
          this.enqueueText(
            peerId,
            '⚠️ 上次的会话暂时没能恢复，已新建会话继续。原会话记录仍在，/sessions 可查看。',
            { kind: 'system' },
          )
        }
   ...
   */
  private async pickOwnedSession(peerId: string): Promise<string | null> {
    const boundId = this.peerSessions.get(peerId)
    if (boundId !== undefined && !this.state.isSessionReleased(boundId)) return boundId
    ...
    for (const [sessionId, owner] of this.sessionOwners) {
      if (owner === peerId && !this.state.isSessionReleased(sessionId)) owned.add(sessionId)
    }
    ...
    try {
      const persistence = this.ctx.get('sessionPersistence') as
        ...
        | undefined
      for (const header of (await persistence?.list()) ?? []) createdAt.set(header.id, header.createdAt)
    } catch {

...
export class WechatBridgeNode {
  ...
  async createSession(peerId: string, prompt: string, mode?: string): Promise<void> {
    ...
    const setup = preset
      ...
      : undefined
    // Origin badge is an OPT-IN enhancement: it requires host support (DSH
    // integration patch or native origin extension). Without it — the default
    // for every user — sessions carry no origin and the bridge depends on
    // nothing outside this package. When enabled, any failure (creation
    // rejection or persistence rejection) degrades to a plain session so a
    // host upgrade can never break session creation.
    const useOrigin = this.resolved.originBadge && !this.originBadgeUnsupported
    try {
      const handle = await this.createAgent({ peerId, preset, provider, model, setup, withOrigin: useOrigin })
      const session = handle.agent.session
      if (useOrigin && !(await this.sessionPersisted(session.id))) {
        // Host accepted the header but did not persist it (e.g. integration
        // patch lost after a DSH upgrade) — a ghost session would be invisible
        // in the sidebar. Drop it and recreate without origin.
        this.originBadgeUnsupported = true
        handle.dispose()
        this.enqueueText(peerId, '⚠️ 宿主不支持来源徽标，已降级为普通会话', { kind: 'system' })
        const plain = await this.createAgent({ peerId, preset, provider, model, setup, withOrigin: false })
        this.setActiveSession(peerId, plain.agent.session.id)
        if (prompt) {
          this.rememberUserText(peerId, prompt)
          plain.agent.followup(
            createUserMessage({
        ...
        this.enqueueText(peerId, `✅ 已创建新会话 ${plain.agent.session.id}${preset ? ` · 模式 ${preset}` : ''}，开始处理…`, { kind: 'system' })
        return
      }
      ...
      if (useOrigin && !this.originBadgeUnsupported) {
        // Creation-time rejection (host header validation): degrade once.
        this.originBadgeUnsupported = true
        this.enqueueText(peerId, '⚠️ 宿主不支持来源徽标，已降级为普通会话', { kind: 'system' })
        try {
          const plain = await this.createAgent({ peerId, preset, provider, model, setup, withOrigin: false })
  ...
  private originBadgeUnsupported = false
  ...
  private async createAgent(params: {
    ...
    meta.cwd = this.state.getPrefs(params.peerId).cwd || this.resolved.cwd || process.cwd()
    if (params.preset) meta.agentPreset = params.preset
    // Structured origin badge: DSH surfaces render the 🟢 WeChat marker from
    // the session header when the host supports it (opt-in, degrades safely).
    if (params.withOrigin) meta.origin = 'wechat'
  ...
  private async sessionPersisted(sessionId: string): Promise<boolean> {
    ...
    for (let attempt = 0; attempt < 10; attempt++) {
      try {
        ...
        if (headers.some((h) => h.id === sessionId)) return true
      } catch {
        return true // persistence unavailable: do not block session creation
      }

...
export function discoverPresets(presetsDir: string = resolveAgentPresetsDir()): PresetInfo[] {
...
export class PresetRegistry {
  private cache: PresetInfo[] | null = null
  ...
  resolveMode(explicit?: string, defaultMode?: string): string | undefined {
    ...
    if (explicit && presets.some((p) => p.id === explicit)) return explicit
    if (defaultMode && presets.some((p) => p.id === defaultMode)) return defaultMode
    return undefined
  }
}

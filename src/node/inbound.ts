...
async function handleImages(
  ...
  for (let i = 0; i < images.length; i++) {
    try {
      ...
      const file = path.join(dir, `wechat-${sanitizeMessageId(message.message_id, Date.now())}-${i}.${ext}`)
...
/**
 * Sanitize a message-derived stamp before it is interpolated into a file name.
 * The gateway's `message_id` is server-issued and normally numeric, but it is
 * attacker-influenced path data for the media-dir join — a malformed id like
 * "/../../x" must never escape the session media directory. Any non-safe char
 * collapses to the numeric fallback, so `path.join` always stays inside `dir`.
 */
function sanitizeMessageId(raw: unknown, fallback: number): string {
  const s = String(raw ?? '').trim()
  return /^[0-9a-zA-Z_-]{1,64}$/.test(s) ? s : String(fallback)
}
...
async function handleMediaFiles(
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    try {
      ...
      const stamp = sanitizeMessageId(message.message_id, Date.now())
      let fileName: string
      if (entry.kind === 'file') {

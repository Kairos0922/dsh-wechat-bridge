...
export function classifyPollBatch(batch: { ret?: number; errcode?: number; errmsg?: string }): PollBatchClass {
  ...
  if (cls === 'session-expired' || cls === 'stale-session') return 'session-expired'
  if (cls === 'rate-limit') return 'rate-limit'
 ...
 */
export function assertCdnUrl(rawUrl: string, extraTrustedHosts?: readonly string[]): URL {
  let parsed: URL
  try {

 ...
 *
 * Seen message ids are persisted here with a TTL as a second safety net for
 * the gateway's account-tagged, atomic PollCursorStore (poll-cursor.json).
 * If a cursor is lost or the server redelivers after reconnect, feeding the
 * same event to agents twice is a correctness bug (double tool execution).
 *
